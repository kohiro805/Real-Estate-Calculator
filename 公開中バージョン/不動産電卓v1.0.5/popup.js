(() => {
  ...
  const render = () => {
    ...
    requestAnimationFrame(() => {
      // 下の段（計算結果）は常に枠に収めたいため、サイズ調整を維持
      if (resDiv.textContent) adjustFontSize(resDiv, 52, 16);
      
      // 上の段（計算式）のロジックを変更
      if (justEvaluated) {
        // 計算終了時（=を押した後）のみ、式全体が見えるよう小さく調整
        adjustFontSize(expDiv, 24, 16);
      } else {
        // 【修正ポイント】入力中はフォントサイズを 38px に固定
        // これにより、サイズが勝手に変わる（明滅する）のを防ぎます
        expDiv.style.fontSize = '38px'; 
      }
      
      // 右端（最新の入力）にスクロールを合わせる
      if (expDiv) {
  ...
  const saveState = () => {
    const state = {
      ...
      revenueInputs: {
        ...
        settingsAcquisitionRate: revenueInputs.settingsAcquisitionRate.value
      },
      valuationInputs: {
  ...
  const formatDisplayValue = (value, threshold = 21, expDecimals = 5) => {
    ...
    const upperLimit = Math.pow(10, threshold);
    const lowerLimit = Math.pow(10, -7);

    if (absNum >= upperLimit || absNum < lowerLimit) {
      return num.toExponential(expDecimals);
    }
    ...
    return num.toLocaleString('en-US', {
      maximumFractionDigits: Math.min(10, Math.max(0, decimalPlaces))
    });
  };
  ...
  const evaluate = () => {
    if (constantOperator) {
      ...
      if (/[×÷+-]/.test(lastChar)) {
        ...
        valToUse = Number(current.slice(0, -1).replace(/,/g, ''));
      } else {
        valToUse = Number(current.replace(/,/g, ''));
      }
    ...
    try {
      let normalized = current.replace(/×/g, '*').replace(/÷/g, '/').replace(/,/g, '').replace(/\s+/g, '');
      
      // 末尾に演算子 (+, -, *, /) が残っている場合、それらを削除する
      while (normalized.length > 0 && /[+\-*/]/.test(normalized.slice(-1))) {
      ...
      current = formatDisplayValue(result, 100);
      justEvaluated = true;
      saveState();
    } catch {
  ...
  const revenueInputs = {
    ...
    settingsAcquisitionRate: document.getElementById('settingsAcquisitionRate')
  };
  ...
  const openValuationSettingsBtn = document.getElementById('openValuationSettingsBtn');
  const closeValuationSettingsBtn = document.getElementById('closeValuationSettingsBtn');
  const valuationMainView = document.getElementById('valuationMainView');
  const valuationSettingsView = document.getElementById('valuationSettingsView');

  const openRevenueSettingsBtn = document.getElementById('openRevenueSettingsBtn');
  const closeRevenueSettingsBtnBottom = document.getElementById('closeRevenueSettingsBtnBottom');
  const revenueMainView = document.getElementById('revenueMainView');
  const revenueSettingsView = document.getElementById('revenueSettingsView');

  if (openRevenueSettingsBtn && revenueMainView && revenueSettingsView) {
    openRevenueSettingsBtn.addEventListener('click', () => {
      revenueMainView.style.display = 'none';
      revenueSettingsView.style.display = 'flex';
    });
  }

  const closeRevSettings = () => {
    revenueSettingsView.style.display = 'none';
    revenueMainView.style.display = 'flex';
  };
  if (closeRevenueSettingsBtnBottom && revenueMainView && revenueSettingsView) {
    closeRevenueSettingsBtnBottom.addEventListener('click', closeRevSettings);
  }
  
  if (openValuationSettingsBtn && valuationMainView && valuationSettingsView) {
    openValuationSettingsBtn.addEventListener('click', () => {
      valuationMainView.style.display = 'none';
      valuationSettingsView.style.display = 'flex';
    });
  }

  if (closeValuationSettingsBtn && valuationMainView && valuationSettingsView) {
    closeValuationSettingsBtn.addEventListener('click', () => {
      valuationSettingsView.style.display = 'none';
      valuationMainView.style.display = 'flex';
    });
  }
  ...
  const updateRevenueOutputs = () => {
    ...
    const acquisitionRate = (parseFloat(revenueInputs.settingsAcquisitionRate.value) || 8) / 100;
    const purchaseExpenses = price * acquisitionRate;
